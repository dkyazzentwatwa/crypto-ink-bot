...
class CryptoWatcher:
    ...
    def __init__(self, cache_ttl: int = 60, exchange_id: str = "binanceus"):
        """
        Initialize crypto watcher.

        Args:
            cache_ttl: Cache time-to-live in seconds (default 60s)
            exchange_id: Exchange to use for OHLCV data (binanceus, kraken, coinbase, bybit)
        """
    ...
    async def __aenter__(self):
        ...
        # Try to initialize exchange, with fallback options
        try:
            exchange_class = getattr(ccxt, self.exchange_id)
            self._exchange = exchange_class({"enableRateLimit": True})
            logger.info(f"Using {self.exchange_id} for OHLCV data")
        except Exception as e:
            logger.warning(f"Failed to initialize {self.exchange_id}: {e}")
            # Try fallback exchanges
            for fallback in ["kraken", "coinbase", "bybit"]:
                if fallback != self.exchange_id:
                    try:
                        exchange_class = getattr(ccxt, fallback)
                        self._exchange = exchange_class({"enableRateLimit": True})
                        logger.info(f"Using fallback exchange: {fallback}")
                        break
